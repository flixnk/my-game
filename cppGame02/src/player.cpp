...
void Player::animate(Vector2 renderPos) {
  ...
  Rectangle dest = {renderPos.x, renderPos.y, frameWidth,
    ...
    if (isFreeHead && isFreeFoot)
      ...
      for (int i = 1; i <= checkUpHeight; i++) {
        ...
        if (isValid(newX, checkUpY) && !map[newX][checkUpY].isSolid) {
          ...
          bool isFreeLeft = isValid(checkAboveLeft, checkAbove) &&
                            !map[checkAboveLeft][checkAbove].isSolid;
          bool isFreeRight = isValid(checkAboveRight, checkAbove) &&
                             !map[checkAboveRight][checkAbove].isSolid;

          // Checking if there is a block above the player
          if (!isFreeLeft || !isFreeRight)
            break;
  ...
  for (int i = 0; i < steps; i++) {
    ...
    if (direction > 0) {
      ...
      if ((isValid(position.x, feetPos) && map[position.x][feetPos].isSolid) ||
          (isValid(position.x + playerSize.x - 1, feetPos) &&
           map[position.x + playerSize.x - 1][feetPos].isSolid)) {
        hitSolid = true;
      }
    } else {
